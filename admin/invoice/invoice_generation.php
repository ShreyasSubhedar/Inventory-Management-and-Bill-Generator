...
<?php
if (isset($_POST['add_newTransaction'])) {
  ...
  if ($check_flag == -1) {
    ...
    header('Location: ../transaction.php?source=add-transaction'); // Redirecting To Add transaction page
  } else {
    //$total_product = count($_POST["product_id"]);
    $sub_total = 0;
    $net_total = 0;
    ...
    for ($i = 1; $i <= $total_product; $i++) {
      ...
      //code to subtract quantity from product table which are sold
      $fetch_available_quantity = $fetch_price_and_quantity['product_quantity'];
      $remaining_quantity = (int) $fetch_available_quantity - (int) ${"quantity" . $i};
      $update_quantity_query = "update product set product_quantity='$remaining_quantity' where product_id='${'product_id' .$i}'";
      $execute_update_quantity_query = mysqli_query($connection, $update_quantity_query);
      //code ends here
    ...
    $next_tid_query = "SELECT AUTO_INCREMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'ProCure' AND TABLE_NAME = 'transaction'";
    $next_tid_fetch = mysqli_fetch_assoc(mysqli_query($connection, $next_tid_query));
    $next_tid = $next_tid_fetch['AUTO_INCREMENT'];
    $order_id = 'OD' . str_replace('-', '', $date) . $next_tid;
    $invoice_id = 'PC' . date("Y", strtotime($date)) . $next_tid;
    $due = $net_total - $paid;

    //now add data to transaction table
    $date_ymd = date("Y-m-d", strtotime($date));
    $add_transaction_query = "insert into transaction (order_id,invoice_id,customer_id,date,sub_total,gst,discount,net_total,paid,due) ";
    $add_transaction_query .= "values('$order_id','$invoice_id','$customer_id','$date_ymd','$sub_total','$gst','$discount','$net_total','$paid','$due')";
    $add_transaction = mysqli_query($connection, $add_transaction_query);
    if (!$add_transaction) {
      echo mysqli_error($connection);
    } else {
      //add data to transaction2 table which has product
      $no_error = 1;
      for ($i = 1; $i <= $total_product; $i++) {
        $add_transaction2_query = "insert into transaction2 values('$next_tid','${"product_id" .$i}','${"quantity" .$i}')";
        $add_transaction2 = mysqli_query($connection, $add_transaction2_query);
        if (!$add_transaction2) {
          echo mysqli_error($connection);
          $no_error = 0;
        }
      ...
      if ($no_error == 1) {
        // echo "Transaction added. <a href='transaction.php'>View Transaction</a>>";
      }
    }
  }
}
...
$BaddressLine1 = substr($row['customer_billingAddress'], 0, 40) . "-";
$BaddressLine2 = substr($row['customer_billingAddress'], 40);
$customer_teamName =$row['customer_teamName'];
$billing_address = array(
  "Billing Address:",
  "Team " . $row['customer_teamName'] . " " . $row['customer_firstName'] . " " . $row['customer_lastName'],
  $BaddressLine1,
  $BaddressLine2,
...
$pdf->SetXY($x + 50, $y);
$x = $x + 50;
$pdf->Cell(44, 7, "Rs " . number_format($net_total, 2), 0, 0, "R", TRUE);
